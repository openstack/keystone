...
class Identity(object):
    ...
    class Tenants:
            ...
            if 'CONTENT_TYPE' in request.environ:
                ...
                if content in content_types:
                    if content == 'application/json':
                        ...
                        cur = con.cursor()
                        cur.execute(
                                    "UPDATE tenants SET tenant_desc='%s' \
                                     WHERE tenant_id='%s'" %\
                ...
                if content in content_types:
                        if content == 'application/json':
                            if tenantId:
                                    ...
                                    cur = con.cursor()
                                    cur.execute("SELECT * FROM tenants WHERE \
                                    ...
                                    cur.execute("SELECT * FROM groups WHERE \
                                                 tenant_id='%s' AND\
                                                 group_id='%s'"\
                                                 % (tenantId, group_id))
                                    a = cur.fetchone()
                                    ...
                                    if a is not None:
                                        return  abort(409, "Group already\
                                                      exists")
                ...
                if content in content_types:
                        if content == 'application/json':
                            if tenantId and groupId:
                                ...
                                cur = con.cursor()
                                # Finding group Exists or not
                                tenant = cur.execute("SELECT * FROM tenants\
                                                WHERE tenant_id='%s'"\
                                                % (tenantId))
                                t = cur.fetchone()
                                resp = ''
                        ...
                        if content == 'application/json':
                            if tenantId and groupId:
                                ...
                                if t is not None:
                                    ...
                                    if group is not None:
                                        cur.execute("DELETE FROM groups WHERE \
                                                     tenant_id='%s' AND \
                                                     group_id='%s' " \
                                                     % (tenantId, groupId))
                                        con.commit()
                        ...
                        if content == 'application/json':
                            if tenantId:
                                ...
                                if t is not None:
                                    ...
                                    groups_exec = cur.execute("SELECT * FROM\
                                                    groups WHERE\
                                                    tenant_id='%s'"\
                                                    % (tenantId))
                        ...
                        if content == 'application/json':
                            if tenantId and groupId:
                                ...
                                if count:
                                    ...
                                    users = cur.execute("SELECT u.* FROM users\
                                                    u INNER JOIN user_group ug\
                                                     ON u.id=ug.user_id WHERE \
                                                    ug.group_id='%s' "
                                                    % (groupId))
                        ...
                        if content == 'application/json':
                            ...
                            if tenantId and groupId and username:
                                ...
                                if t is not None:
                                    ...
                                    cur.execute(
                                       "INSERT INTO user_group(user_id,\
                                       group_id) \
                                       VALUES ('%s','%s')" \
                                       % (username, groupId))
                                    con.commit()
        ...
        @route('/v1.0/tenants/:tenantId/groups/:groupId/users', \
               method='DELETE')
        ...
        def create_user(tenantId):
            ...
            if 'CONTENT_TYPE' in request.environ:
                ...
                if content in content_types:
                    ...
                    cur.execute("SELECT COUNT(*) FROM users WHERE \
                                id='%s'" % (username))
                    username_conflict = cur.fetchone()
            ...
            if 'CONTENT_TYPE' in request.environ:
                ...
                if content in content_types:
                        ...
                        if result is not None:
                            ...
                            accept_header = request.header.get('Accept')
                            token = hashlib.sha224(str(username + password) +\
                                    str(datetime.now())).hexdigest()[:21]
                            expires = datetime.now() + timedelta(minutes=1)
                            con = sqlite3.connect(dbpath,\
                                        detect_types=sqlite3.PARSE_DECLTYPES)
                            cur = con.cursor()
                            cur.execute(
                                        'INSERT INTO token(token_id, expires, \
                                         user_id) VALUES(?, ?, ?)', (token, \
                                          expires, username))
                            ...
                            if accept_header in content_types:
                                ...
                                # If there is no Accept header, the default
                                # is JSON
                                #
                                #TODO:
                                # Make sure that the body is actually JSON.
            ...
            if 'CONTENT_TYPE' in request.environ:
                ...
                if content in content_types:
                        ...
                        if row is None:
                            ...
                            if (int(result['enabled']) == 0):
                                return abort(403, "UserDisabled")
                            expires = datetime.strptime(row['expires'],\
                                        "%Y-%m-%d %H:%M:%S.%f")
                            if(expires < datetime.now()):
                                abort(401, "Token Expired")
                            else:
                                groups = cur.execute("SELECT * FROM \
                                            groups WHERE tenant_id = '%s'" \
                                            % (tenantid))
                                resp = ''
                                if groups.rowcount > 100:
                                    return abort(413, "Over Limit")
                                ...
                                return '{"auth" : { "token": {"id": "%s", \
                                        "expires": "%s"}, "user" :{"%s", \
                                        "username": "%s", "tenantId": "%s"}}}'\
                                        % (str(row['token_id']), \
                                        str(row['expires']),\
                                        resp, user, tenantid)
            ...
            if 'CONTENT_TYPE' in request.environ:
                ...
                if content in content_types:
                    ...
                    if content == 'application/json':
                        ...
                        return "{'version':'%s'}" % config.get('DEFAULT', \
                                                                'version')
