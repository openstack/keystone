...
class TestKeystoneFlaskCommon(rest.RestfulTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(
            fixtures.MockPatchObject(enf, 'register_rules', register_rules)
        )
        self.useFixture(
                ...
                'identity': {
                    ...
                    'password': {
                        'user': {
                            ...
                            'domain': {'id': self.user_req_admin['domain_id']},
                        }
                    },
                },
                'scope': {'project': {'id': self.project_service['id']}},
            }
        }
        return (
            self.test_client()
            .post('/v3/auth/tokens', json=auth_json, expected_status_code=201)
    ...
    def _setup_flask_restful_api(self, **options):
        ...
        setattr(
            ...
            'api_prefix',
            options.get('api_url_prefix', ''),
        )
        self.addCleanup(
            setattr, _TestResourceWithCollectionInfo, 'api_prefix', orig_value
        )
    ...
    def _make_requests(self):
        ...
        with self.test_client() as c:
            ...
            self.assertEqual(
                _TestResourceWithCollectionInfo.wrap_collection([]), resp.json
            )
            ...
            new_argument_resp = c.post(
                url, json=new_argument_resource, headers=headers
            ).json['argument']

            # POST second new ref
            new_argument2_resource = {'testing': uuid.uuid4().hex}
            new_argument2_resp = c.post(
                url, json=new_argument2_resource, headers=headers
            ).json['argument']
    ...
    def test_construct_resource_map(self):
        ...
        self.assertEqual(
            json_home.build_v3_resource_relation(resource_name),
            mapping.json_home_data.rel,
        )
        self.assertEqual(
            json_home.Status.EXPERIMENTAL, mapping.json_home_data.status
        )
        self.assertEqual(
    ...
    def test_unenforced_api_decorator(self):
        ...
        restful_api = _TestRestfulAPI(
            resource_mapping=[resource_map], resources=[]
        )
        ...
        with self.test_client() as c:
            ...
            # Works with token
            resp = c.post(
                '/v3/test_api', json=body, headers={'X-Auth-Token': token}
            )
    ...
    def test_normalize_domain_id_extracts_domain_id_if_needed(self):
        ...
        with self.test_client() as c:
            ...
            self.assertEqual(
                oslo_context.domain_id, ref_without_domain_id['domain_id']
            )
            ref_without_domain_id.clear()
    ...
    def test_json_body_before_req_func_invalid_json(self):
        with self.test_request_context(
            ...
            self.assertRaises(
                exception.ValidationError, json_body.json_body_before_request
            )
