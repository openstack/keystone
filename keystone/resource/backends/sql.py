    ...
    def get_project_by_name(self, project_name, domain_id):
        with sql.session_for_read() as session:
            ...
            if domain_id is None:
                query = query.filter_by(domain_id=base.NULL_DOMAIN_ID)
            else:
    ...
    def list_project_ids_from_domain_ids(self, domain_ids):
        if not domain_ids:
            ...
            with sql.session_for_read() as session:
                ...
                query = query.filter(
                    sql_model.Project.domain_id.in_(domain_ids)
                )
                return [
                    x.id for x in query.all() if not self._is_hidden_ref(x)
                ]
            ...
            while children:
                ...
                for ref in children:
                    if ref['id'] in examined:
                        msg = (
                            'Circular reference or a repeated '
    ...
    def list_projects_by_tags(self, filters):
        ...
        with sql.session_for_read() as session:
            ...
            if 'not-tags' in filters.keys():
                ...
                filtered_ids = self._filter_not_tags(
                    session, filtered_ids, blacklist_ids
                )
    ...
    def delete_projects_from_ids(self, project_ids):
        ...
        with sql.session_for_write() as session:
            ...
            for project_id in project_ids:
                if (
                    project_id not in project_ids_from_bd
